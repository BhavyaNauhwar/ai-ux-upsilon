  ...
  for (const part of parts) {
    ...
    for (const chunk of part.split(/\n\n+/)) {
      ...
      if (heading && lines.length === 1) {
        ...
        blocks.push({ type: "quote", content: lines.map((l) => l.replace(/^\s*>\s?/, "")).join(" ") });
      } else if (
        lines.length >= 2 &&
        lines[0].includes("|") &&
        /^\s*\|?\s*:?-{2,}/.test(lines[1])
    ...
    case "chats":
      return (
        <SettingsGroup title="Chats">
          ...
          <SettingRow label="Autotitle new chats" description="Generate a title from the first message."><Switch defaultChecked /></SettingRow>
        </SettingsGroup>
      );
    case "about":
      return (
        <SettingsGroup title="About">
          <SettingRow label="Version" description="LexMed AI web client.">
...
function SettingsGroup({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section>
      <h3 className="mb-3 text-[12px] font-semibold uppercase tracking-[0.1em] text-foreground/50">{title}</h3>
  ...
  return (
    <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        ...
        {description && <div className="text-[12.5px] text-foreground/55">{description}</div>}
      </div>
...
function ImageLightbox({ url, onClose }: { url: string | null; onClose: () => void }) {
  ...
  const download = async () => {
    try {
      ...
      a.href = href;
      a.download = `lexmed-image-${Date.now()}.jpg`;
      a.click();
